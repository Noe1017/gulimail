    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.schedName) {
            this.$http({
              url: this.$http.adornUrl(`/ware/qrtzschedulerstate/info/${this.dataForm.schedName}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ware/qrtzschedulerstate/${!this.dataForm.schedName ? 'save' : 'update'}`),
              method: 'post',
